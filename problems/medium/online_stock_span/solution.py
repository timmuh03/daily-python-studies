"""
Docstring for problems.medium.online_stock_span.solution
...
class StockSpanner:
    ...
    def __init__(self):
        self.blockers = [] # Carry potential (unresolved) blockers across each call

    def next(self, price: int) -> int:
        span = 1
        # Absorb consecutive blocks with price <= today's price
        while self.blockers and self.blockers[-1][0] <= price:
            _, prev_span = self.blockers.pop()
            span += prev_span
            
        self.blockers.append((price, span)) # today's price is always a potential blocker
        return span
